...
use crate::{
    axvisor::{
        cases::{
            RunArtifacts,
            guest_build::{CaseBuildInfo, load_case_build_info, resolve_build_info_path_in_dir},
            manifest::LoadedCase,
            rootfs,
        },
...
async fn build_guest_case(
    ...
    let mut build_info: CaseBuildInfo = load_case_build_info(build_info_path, target)?;
    build_info.log = ostool::build::config::LogLevel::Warn;
    let cargo = build_info.into_prepared_cargo_config(package, target, None)?;
